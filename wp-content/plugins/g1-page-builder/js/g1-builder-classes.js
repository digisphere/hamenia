...
(function ($, context) {
    ...
    var g1BuilderParser = function () {
        ...
        function convertFirstLevelShortcodesToHtml (text, dontTraverseDeeper) {
            ...
            $set.each(function () {
                ...
                if (html.indexOf('[toggle') === 0 || html.indexOf('[box') === 0 || html.indexOf('[carousel_item') === 0 || html.indexOf('[tab_title') === 0 || html.indexOf('[tab_content') === 0) {
    ...
    var converterClass = function () {
        var that = {}; // defines public scope
        var patterns;
        var allowedShortcodes = context.getListOfRegisteredElements();  // page builder block list
        ...
        that.toHtml = function (text, listOfShortcodesToConvert) {
            if (listOfShortcodesToConvert) {
                allowedShortcodes = listOfShortcodesToConvert;
            }

            text = prepareSpecialShortcodes(text);
            text = normalizeShortcodeTags(text);
            text = flattenNestedShortcodes(text);
        ...
        that.toText = function (shortcodeId, fields, options) {
            var content = g1.getValueByKey(fields, 'content', null);
            var displayType = g1.getValueByKey(options, 'displayType', 'block');

            if (content) {
                delete fields.content;
            }

            return composeTextShortcode(shortcodeId, displayType, fields, content);
        };
        ...
        function composeTextShortcode (name, displayType, attributes, content) {
            ...
            if (content) {
                switch (displayType) {
                    case 'block':
                        ...
                        out += newLineSeparator + content + newLineSeparator + '[/' + name + ']';
        ...
        function replaceShortcodeTagsWithHtmlTags (text) {
            // wrap all shortcodes into html to parse them as xml
            text = wrapShortcodeTagsIntoHtmlHolders(text);
        ...
        function getInlineShortcodesList () {
            var inlineShortcodes = ['button'];

            if (typeof g1_page_builder_inline_shortcodes_list === 'function') {
                inlineShortcodes = g1_page_builder_inline_shortcodes_list(inlineShortcodes);
            }

            return inlineShortcodes;
        }

        function replaceInlineShortcodesToNoneShortcodes (text) {
            var inlineShortcodes = getInlineShortcodesList();

            for (var i = 0; i < inlineShortcodes.length; i++) {
                var shortcodeName = inlineShortcodes[i];
                var openPatt = new RegExp('\\[' + shortcodeName, 'g');
                var closePatt = new RegExp('\\[\\/' + shortcodeName, 'g');

                text = text.replace(openPatt, '(' + shortcodeName);
                text = text.replace(closePatt, '(/' + shortcodeName);
            }

            return text;
        }

        function replaceNoneShortcodesToInlineShortcodes (text) {
            var inlineShortcodes = getInlineShortcodesList();
        ...
        function wrapShortcodeTagsIntoHtmlHolders (out) {
            // inline shortcodes shoudn't be converted so we change them to none shortcode for a time of conversion
            out = replaceInlineShortcodesToNoneShortcodes(out);

            out = out.replace(/(\[\/[^\]]+\])/ig, '</section>');
            out = out.replace(/\[([^\/]{1}[^\]]+)\]/ig, '<section class="g1-temp-shortcode-wrapper" data-g1-sc-name=$1>');

            // revert inline shortcodes to shortcode form
            out = replaceNoneShortcodesToInlineShortcodes(out);

            // protect shortcode "class" attribute from being escaped by browser parser (section has its own "class" attr but can be only one)
            out = out.replace(/(data\-g1\-sc\-name=[^>]*)(class=)/g, '$1g1-sc-attr-$2');

            return out;
        }

        function extractShortcodeFromHtmlHolder ($shortcode) {
            var shortcodeName = $shortcode.attr('data-g1-sc-name');
            var shortcode = $shortcode[0].outerHTML.replace('<section class="g1-temp-shortcode-wrapper" data-g1-sc-name=', '');

            // restore real "class" attr
            shortcode = shortcode.replace('g1-sc-attr-class', 'class');

            var endIndex = shortcode.indexOf('>');
            shortcode = shortcode.slice(0, endIndex);
            shortcode = '[' + shortcode.replace('"'+ shortcodeName +'"', shortcodeName) + ']';

            return {
                'openTag': shortcode,
                'closeTag': '[/'+ shortcodeName +']'
            };
        }

        function replaceAllShortcodeWrappersWithText ($wrapper) {
            var removeWrappers = function () {
                $wrapper.find('.g1-temp-shortcode-wrapper').each(function () {
                    replaceShortcodeWrapperWithText($(this));
                });
            };

            // runs as many times as deep is DOM tree
            while ($wrapper.find('.g1-temp-shortcode-wrapper').length > 0) {
                removeWrappers();
            }
        ...
        function replaceShortcodeWrapperWithText ($shortcode) {
            var shortcode = extractShortcodeFromHtmlHolder($shortcode);

            $shortcode.replaceWith(shortcode.openTag + $shortcode.html() + shortcode.closeTag);
        }

        function replaceShortcodeWrapperWithHtml ($shortcode, recursive) {
            // by default we want to recursively parse content
            ...
            var shortcodeHtmlOpenTag = convertShortcodeToHtml(shortcode.openTag);
            var shortcodeHtmlCloseTag = convertShortcodeToHtml(shortcode.closeTag);

            var shortcodeContent = $shortcode.html();

            if (recursive) {
                shortcodeContent = convertFirstLevelShortcodesToHtml(shortcodeContent);
            }

            $shortcode.replaceWith(shortcodeHtmlOpenTag + shortcodeContent + shortcodeHtmlCloseTag);
        }

        function convertFirstLevelShortcodesToHtml (text) {
            // add wrapper to allow traversing over child elements
            var $wrappedText = $('<section class="g1-traverse-wrapper">' + text + '</section>');

            // find only top level shortcodes, nested will be processed recursively
            var $firstLevelShortcodes = $wrappedText.find('> .g1-temp-shortcode-wrapper');

            // if there are no top level shortcodes, convert to text all other nested shortcodes (treated as inline)
            if ($firstLevelShortcodes.length === 0) {
                $wrappedText = replaceAllShortcodeWrappersWithText($wrappedText);

                // return inner html to get rid of g1-traverse-wrapper
                return $wrappedText[0].innerHTML;
            }

            // convert top level shortcodes to HTML blocks that will be recognized by the builder
            $firstLevelShortcodes.each(function () {
                var $shortcode = $(this);
                var shortcodeName = $shortcode.attr('data-g1-sc-name');
                ...
                var isInlineElement = $.inArray(shortcodeName, allowedShortcodes) === -1;

                var parseShortcodeContentRecursively = true;

                // some builder elements can't contain other elements inside. We must leave it as text content
                var omitParsingContentForShortcodes = [
                    'box',
                    'toggle',
                    'tab_title',
                    'tab_content',
                ...
                if (omitParsingContentForShortcodes.indexOf(shortcodeName) !== -1) {
                    parseShortcodeContentRecursively = false;
                }

                if (isInlineElement) {
                    replaceShortcodeWrapperWithText($shortcode);
                } else {
                    replaceShortcodeWrapperWithHtml($shortcode, parseShortcodeContentRecursively);
                }
            });

            // convert to text all other shortcodes
            $wrappedText = replaceAllShortcodeWrappersWithText($wrappedText);
        ...
        function prepareSpecialShortcodes (text) {
            // rev_slider
            text = text.replace(/\[rev_slider\s+([^\]]+)\]/g, '[rev_slider alias="$1"]');
        ...
        function flattenNestedShortcodes (content) {
            // replace
            // [_ [__ [___ etc with single [
            // and
            // [/_ [/__ [/___ etc with single [/
        ...
        function convertShortcodeToHtml (shortcode) {
            ...
            var html = '<section class="g1-shortcode"';
            var attributesOrderedList = [];
            ...
            html += ' data-g1-shortcode-name="'+ shortcodeName +'"';
        ...
        return init(); // gives access to public methods
    };
    ...
    context.classes.g1BuilderParser = converterClass;
