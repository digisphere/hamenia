...
(function ($, context) {
    ...
    var g1Builder = function ($classicEditorWrapper, options) {
        ...
        function convertTextToBuilderElements (textContent) {
            ...
            var htmlContent = parser.toHtml(textContent);
        ...
        function convertBuilderElementsToText () {
            ...
            try {
                ...
                var isEditorInVisualMode = false;

                if (typeof tinyMCE !== 'undefined') {
                    var tinyEditor = tinyMCE.get(editorId);

                    if (typeof tinyEditor === 'object' && tinyEditor !== null) {
                        tinyEditor.focus();
                        isEditorInVisualMode = tinyMCE.activeEditor && !tinyMCE.activeEditor.isHidden();
                    }
                }
                ...
                if (isEditorInVisualMode) {
                    ...
                    tinyEditor.setContent(text, {format: 'raw'});
                ...
                alert('Save failed. Post support ticket with this error message: ' + e.message);
        ...
        function createElementInstance (name, tagName, htmlContent, attrs) {
            ...
            if (!isContentElement && htmlContent) {
                var domNodes = $.parseHTML(htmlContent, document, true);
                ...
                $(domNodes).each(function () {
                    ...
                    if (isNotEmptyNode(node)) {
                        ...
                        var nodeName = isShortcode ? $node.attr('data-g1-shortcode-name') : 'html_block';
                        var nodeTagName = isShortcode ? $node.attr('data-g1-shortcode-name') : null;
