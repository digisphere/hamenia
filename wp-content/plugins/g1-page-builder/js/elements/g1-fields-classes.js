...
(function ($, context) {
    ...
    var richTextField = function (name, $html) {
        ...
        that.closeTags = function () {
            ...
            // switch to rich editor if we're in text mode
            // this will fix all unclosed/bad closed tags
            if ($editor.is('.html-active')) {
                $editor.find('.switch-tmce').trigger('click');
            }

            // switch to text mode on exit
        ...
        function wpEditorConvertNewLines (content) {
            if ( typeof g1_page_builder_wp_editor_conver_new_lines === 'function' ) {
                return g1_page_builder_wp_editor_conver_new_lines(content);
            }

            // textarea field can't have <br> tags,
            // it should be new line (\n) characters (to break line)
        ...
        function switchToTinyEditor () {
            ...
            // Build 1.5 fix - nbsp; added after saving HTML block.
            //var textareaValue = wpEditorConvertNewLines(that.getValue());
            var textareaValue = that.getValue();

            window.tinyMCE
                .get(editorId)
                .setContent(
                window.switchEditors.wpautop(textareaValue),
                {
                    format:'raw'
                }
            );
